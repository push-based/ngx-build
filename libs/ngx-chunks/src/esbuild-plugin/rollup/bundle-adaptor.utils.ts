...
export function rollupToEsbuildOutputs(
  rollupOutput: RollupOutput['output'],
  { outputs }: Metafile,
  reverseLookup: MergeStrategyReverseLookup
) {
  ...
  rollupOutput
    ...
    .forEach((output) => {
      ...
      newMetafileOutputs[output.fileName] = {
        ...
        inputs: mergedInputs(
          Object.keys(output.modules).map((chunk) => outputs[chunk].inputs)
        ),
        entryPoint: mergedEntryPoint(
          reverseLookup
            .get(output.facadeModuleId!)
            ?.map((chunk) => outputs[chunk].entryPoint) ?? []
        ),
...
function mergedEntryPoint(
  mergeChunksEntryPoint: Metafile['outputs'][string]['entryPoint'][]
): Metafile['outputs'][string]['entryPoint'] {
  const entryPoints = mergeChunksEntryPoint.filter(Boolean);
  assert(
    entryPoints.length <= 1,
    'Warning merged chunk contained multiple entryPoints',
    entryPoints.length,
    entryPoints
  );
...
function mergedInputs(
  mergedChunksInputs: Metafile['outputs'][string]['inputs'][]
