...
export function detectMergeCandidates(
  ...
  console.log(
    `🔍 Generating combinations from ${eligibleChunks.length} eligible chunks...`
  );

  // Generate all possible combinations of eligible chunks
  const combinations = generateChunkCombinations(
    eligibleChunks,
    opts.maxChunksPerMerge
  );
  ...
  for (const chunks of combinations) {
    ...
    console.log(
      `🎯 Candidate [${chunks.slice(0, 2).join(', ')}${
        chunks.length > 2 ? '...' : ''
      }]: priority=${candidate.priority}, reason=${candidate.mergeReason}`
    );

    if (candidate.priority > 0) {
      // Only include candidates with positive priority
 ...
 */
function generateChunkCombinations(
  chunks: OutputPath[],
  maxSize: number
): OutputPath[][] {
  ...
  const chunksToConsider = chunks.slice(
    0,
    Math.min(MAX_CHUNKS_TO_CONSIDER, chunks.length)
  );
  ...
  for (
    let size = 2;
    size <= Math.min(MAX_COMBINATION_SIZE, chunksToConsider.length);
    size++
  ) {
 ...
 */
function calculateSharedDependencies(
  chunks: OutputPath[],
  metafile: Metafile
): number {
 ...
 */
function calculateDependencyDistance(
  chunks: OutputPath[],
  metafile: Metafile
): number {
  if (chunks.length < 2) return 0;
 ...
 */
function calculateLoadTimingCorrelation(
  chunks: OutputPath[],
  metafile: Metafile
): number {
 ...
 */
export function calculateMergePriority(
  candidate: Omit<MergeCandidate, 'priority'>
): number {
  let score = 0;
  ...
  if (candidate.combinedSize < maxEfficientSize) {
    score +=
      Math.max(
        0,
        (maxEfficientSize - candidate.combinedSize) / maxEfficientSize
      ) * 40;
  } else {
  ...
  if (candidate.dependencyDistance < maxDistance) {
    const proximityFactor = Math.max(
      0,
      (maxDistance - candidate.dependencyDistance) / maxDistance
    );
  ...
  if (finalScore <= 0) {
    console.log(
      `❌ Zero priority for [${candidate.chunks
        .slice(0, 2)
        .join(', ')}]: size=${candidate.combinedSize}, shared=${
        candidate.sharedDependencyCount
      }, distance=${candidate.dependencyDistance}`
    );
  }
