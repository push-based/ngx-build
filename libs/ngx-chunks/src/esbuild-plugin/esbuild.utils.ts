...
export function importsInEntryPoint(
  ...
  const staticImports = metaFileOutputs[entryPoint].imports.filter(
    ({ kind, path, external }) =>
      kind !== 'dynamic-import' && !traversedImports.includes(path) && !external
  );
  ...
  return staticImports.flatMap(({ path }) =>
    importsInEntryPoint(path, metaFileOutputs, [...traversedImports, path])
  );
}
...
function getChunkNameByEntryPoint(
  ...
  return Object.keys(metafileOutputs).find(
    (name) => metafileOutputs[name].entryPoint === entryPoint
  );
}
...
function getEntryPointPath(entryPoints: BuildOptions['entryPoints']): string {
  if (!entryPoints) {
    throw new Error(
      'Could not extract entryPoints for entryPoints as its undefined'
    );
  }
  if (Array.isArray(entryPoints)) {
    if (entryPoints.length > 1) {
      throw new Error(
        'We currently do not support multiple entry points, if you are interested in this feature please open an issue'
      );
    }
  ...
  throw new Error(
    'Could not extract entryPoints please pass it as an option to the plugin'
  );
}
