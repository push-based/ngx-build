...
describe('MergeCandidateDetector', () => {
  ...
  describe('detectMergeCandidates', () => {
    ...
    it('should prioritize candidates with shared dependencies', () => {
      ...
      const sharedDepsCandidate = candidates.find(
        (c) =>
          c.chunks.length === 2 &&
          c.chunks.includes('chunk1.js') &&
          c.chunks.includes('chunk2.js')
      );

      const noSharedDepsCandidate = candidates.find(
        (c) =>
          c.chunks.length === 2 &&
          c.chunks.includes('chunk2.js') &&
          c.chunks.includes('chunk3.js')
      );
      ...
      if (sharedDepsCandidate && noSharedDepsCandidate) {
        ...
        expect(sharedDepsCandidate.priority).toBeGreaterThan(
          noSharedDepsCandidate.priority
        );
      }
    });
  ...
  describe('calculateMergePriority', () => {
    ...
    it('should give higher priority to shared dependencies', () => {
      ...
      const noSharedDepsPriority = calculateMergePriority(
        noSharedDepsCandidate
      );
  ...
  describe('filterConflictingCandidates', () => {
    it('should filter out candidates that conflict with existing strategy', () => {
      ...
      const filteredCandidates = filterConflictingCandidates(
        candidates,
        existingStrategy
      );
