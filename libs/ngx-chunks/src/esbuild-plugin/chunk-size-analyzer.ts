 ...
 */
export function calculateSizeSummary(
  ...
  const smallChunksSize = eligibleChunks.reduce(
    (sum, info) => sum + info.rawSize,
    0
  );
 ...
 */
function isJavaScriptChunk(
  path: string,
  outputInfo: Metafile['outputs'][string]
): boolean {
