...
export function getStrategyLookup(
  ...
  const lookup: MergeStrategyLookup = new Map(
    [...strategy].flatMap(([key, values]) =>
      values.map((value) => [value, key])
    )
  );
  const reverseLookup: MergeStrategyReverseLookup = new Map(
    [...lookup].map(([key, value]) => [key, strategy.get(value)!])
  );
  return { lookup, reverseLookup, strategy };
}

function getMergeStrategyMap(
  entryPoint: OutputPath,
  { outputs }: Metafile,
  maxBins: number
): MergeStrategyMap {
  ...
  return new Map(
    [...mergedChunkGroups, ...notMergedChunks].map((values) => [
      hashFromOutputPaths(values),
      values,
    ])
  );
}
...
function mergeStrategy(
  ...
  const weights = new Map<OutputPath, number>(
    initialChunks.map((outputPath) => [
      outputPath,
      metafileOutputs[outputPath].bytes,
    ])
  );
  const mergeMap = new Map<OutputPath, MergeKey[]>(
    sortedChunks.map((chunk) => [chunk, [chunk]])
  );
  ...
  while (sortedChunks.length > maxBins) {
    ...
    weights.set(
      sortedChunks[x],
      weights.get(sortedChunks[x])! + weights.get(sortedChunks[y])!
    );
...
function findMergeTargets(
  sortedChunks: OutputPath[],
  weights: Map<OutputPath, number>
): [number, number] {
  let candidateWeight = Infinity;
  let candidateIndex = 0;
  for (let i = 0; i < sortedChunks.length - 1; i++) {
    const mergedWeight =
      weights.get(sortedChunks[i])! + weights.get(sortedChunks[i + 1])!;
