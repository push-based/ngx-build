...
export function mergeStrategyV2(
  ...
  const rootGroup = staticClosure(
    assertKnownOutput(rootEntryPoint, allOutputPaths)
  );
  ...
  const enqueueDynRootsFrom = (
    fromSet: Set<OutputPath>,
    parentKey?: OutputPath
  ) => {
  ...
  function assertKnownOutput(p: OutputPath, known: Set<string>): OutputPath {
    if (!known.has(p)) {
      ...
      const byEntry = Object.entries(outputs).find(
        ([, info]) => info.entryPoint === p
      )?.[0];
